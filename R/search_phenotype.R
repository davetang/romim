...
#' @export
#' @examples
#' search_phenotype(search_term = 'noonan')

search_phenotype <- function(search_term = 'noonan'){
  search_term <- gsub("\\s", '%20', search_term)
  search_term <- sub("^", "%22", search_term)
  search_term <- sub("$", "%22", search_term)
  my_search   <- paste('http://api.omim.org/api/entry/search?search=', search_term, '+AND+gm_phenotype_exists:true', sep='')
  ...
  if (my_total < 21){
    ...
    if (my_total > 100){
       return(paste('Search term: ', search_term, ' returned over 100 results (', my_total, ')', sep = ''))
    }
